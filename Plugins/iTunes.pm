...
my %functions = (
	'up' => sub  {
		my $client = shift;
		Slim::Utils::Prefs::set('itunes',Slim::Buttons::Common::scroll($client, -1, 2, Slim::Utils::Prefs::get('itunes')));
		$client->update();
	},
	'down' => sub  {
		my $client = shift;
...
sub lines {
	my $client = shift;
	my ($line1, $line2);
	$line1 = $client->string('SETUP_ITUNES');
	if (Slim::Utils::Prefs::get('itunes')) {
		$line2 = $client->string('USE_ITUNES');
	} else {
		$line2 = $client->string('DONT_USE_ITUNES');
	};
	return ($line1, $line2);
}
