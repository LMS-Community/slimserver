...
if (main::LIBRARY) {
	require Slim::Control::Library::Commands;
}
...
sub playlistXitemCommand {
	...
	if ( main::LIBRARY && $path =~ /^db:|^itunesplaylist:|^musicipplaylist:/ ) {
	...
	if ( main::LIBRARY && !Slim::Music::Info::isRemoteURL($path) && !-e $path && !(Slim::Music::Info::isPlaylistURL($path)) ) {
...
sub playlistXtracksCommand {
	...
	} elsif (main::LIBRARY && $what =~ /searchRef/i) {
...
sub playlistcontrolCommand {
	...
	if ( main::LIBRARY && defined(my $playlist_name = $request->getParam('playlist_name')) ) {
