...
sub processCoverArtRequest {
	...
	$path =~ /music\/(\w+)\/(cover|thumb)(?:_(X|\d+)x(X|\d+))?(?:_([sSfFpc]))?(?:_([\da-fA-F]{6}))?\.jpg$/;
	...
	my $requestedBackColour = defined($6) ? hex $6 : 0xFFFFFF; # bg color used when padding
	...
	# If this is a thumb, a size has been given, or this is a png (and may have an alpha channel)
	# then the overhead of loading the image with GD is necessary.  Otherwise, the original content
	# can be passed straight through.

	if ($canUseGD && ($image eq "thumb" || $requestedWidth || $contentType eq "image/png")) {
		...
		$::d_http && msg("can't use GD or no need to process image\n");
		$body = \$imageData;
	}
