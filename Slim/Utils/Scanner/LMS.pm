...
sub new {
	...
	for my $thumb ( @{ $result->thumbnails } ) {
		my $cached = {
			content_type  => $result->codec eq 'JPEG' ? 'jpg' : 'png',
			mtime         => $result->mtime,
		...
		my $key = 'music/' . $result->hash . '/cover_300x300_o';
		Slim::Utils::ArtworkCache->new->set( $key, $cached );
		
		main::INFOLOG && $log->is_info && !(main::SCANNER && $main::progress) && $log->info("Cached thumbnail for $key");
