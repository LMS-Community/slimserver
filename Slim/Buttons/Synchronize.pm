...
# $Id: Synchronize.pm,v 1.1 2003/07/18 19:42:13 dean Exp $
	...
	if (scalar @{$client->syncSelections} < 1) {
		...
		Slim::Buttons::Common::popMode($client);
			...
			while (scalar(@buddies) > 2) {
				...
				$line2 .= Slim::Player::Client::name($buddy) . ", ";
			}
			
			if (scalar(@buddies) > 1) {
				my $buddy = pop @buddies;
				$line2 .= Slim::Player::Client::name($buddy) . " " . Slim::Utils::Strings::string('AND') . " ";		
			}
			
			$line2 .= Slim::Player::Client::name(pop @buddies);
	}
