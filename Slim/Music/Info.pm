	...
	if (isSong($file, $type) ) {
		if (isHTTPURL($file)) {
			...
			if (exists $tempCacheEntry->{'COVER'} || exists $tempCacheEntry->{'THUMB'}) {
				...
				my ($body,$contenttype,$path) = readCoverArt($filepath,'cover');
				if (defined $body) {$tempCacheEntry->{'COVER'} = $path;}
				# look for Thumbnail Art and cache location
				($body,$contenttype,$path) = readCoverArt($filepath,'thumb');
				if (defined $body) {
...
sub readCoverArt {
	...
	if (isSong($filepath) && isFile($filepath)) {
		...
		if ($body) {
			...
			($body,$contenttype,$cover) = readCoverArtFiles($file,$image);
		}
 	}
...
sub updateArtworkCache {
	...
	if (defined $thumb && defined $album && $thumb) {
		if (!exists $artworkCache{$album}) { # only cache albums once each
			...
			$::d_info && Slim::Utils::Misc::msg("Updating $album artwork cache: $filepath\n");
			$artworkCache{$album} = $filepath;
		}
	}
}
