...
# we include with the server, first checking our CPAN path, then if
...
*******
	...
	if ( scalar keys %{$failed} ) {
		...
		print "The following CPAN modules were found but cannot work with the server:\n";
		...
		print "3. Run the .tar.gz version of UE Music Library which includes all required CPAN modules.\n";
		print "\n";
	...
	for my $module (@modules) {
		...
		if ($@) {
			...
			if ($@ =~ /cannot restore segment prot after reloc/) {
				...
				print STDERR "** UE Music Library Error:\n";
				print STDERR "** SELinux settings prevented the server from starting.\n";
