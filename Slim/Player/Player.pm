...
# remain on the display for the proper duration unlike showBriefly which
# can get removed early.
sub pushBriefly {
	my ( $client, $args ) = @_;
	...
	my $length;	
	my $size = $client->measureText( $line2, 2 );
	
	# Calculate a sane default display time
	
	# if display width is big enough to hold the message, display for 3 secs
	if ( $size <= $client->displayWidth ) {
		$length = 3;
	}
	else {
		# adjust timer length based on client font size
		if ( !$client->prefGet('activeFont_curr') ) {
			# small
			$length = int($size / 40);
		}
		elsif ( $client->prefGet('activeFont_curr') == 1 ) {
			# medium
			$length = int($size / 50);
		}
		elsif ( $client->prefGet('activeFont_curr') == 2 ) {
			# large
			$length = int($size / 60);
		}
	}
	
	# User may override the time
	if ( $args->{'duration'} ) {
		$length = $args->{'duration'};
	}
	
	# User-specified coderef to run on end of message
	my $code = $args->{'code'};
