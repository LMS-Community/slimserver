...
sub tokenizeConvertCommand2 {
	...
	$subs{'CLIENTID'}  = do { (my $tmp = $transcoder->{'clientid'}) =~ s/://g;  $tmp };
	$subs{'PLAYER'}    = do { (my $tmp = $transcoder->{'player'}  ) =~ tr/\" /_/; $tmp };
	$subs{'NAME'}      = do { (my $tmp = $transcoder->{'name'}    ) =~ tr/\" /_/; $tmp };
	$subs{'GROUPID'}   = $transcoder->{'groupid'} eq 0 ? $subs{'CLIENTID'} : sprintf ( "g%011x", $transcoder->{'groupid'});
		...
		my $transcoder  = $binaries{$placeholder} || '';
 		...
 		if (!exists $binaries{$placeholder}) {
			...
			if ($file && $pref) {
				...
				$transcoder =~ s/\s+/ /;
				!$transcoder && (preferences($file)->set($pref,' ')); 
			}
 			else {
