...
sub trackInfoHandler {
	...
	if ( !defined $index ) {
		...
		if ( $tags->{menuMode} ) {
			...
			return {
				type        => 'text',
				name        => Slim::Utils::Strings::clientString('JIVE_ADD_TO_FAVORITES'),
				jive        => $jive,
			};
		} else {
			return {
				type        => 'link',
				name        => Slim::Utils::Strings::clientString('PLUGIN_FAVORITES_ADD'),
		...
		if ( $tags->{menuMode} ) {
			...
			return {
				type        => 'text',
				name        => Slim::Utils::Strings::clientString('JIVE_DELETE_FROM_FAVORITES'),
				jive        => $jive,
			};
			...
			return {
				type        => 'link',
				name        => Slim::Utils::Strings::clientString('PLUGIN_FAVORITES_REMOVE'),
...
sub trackInfoAddFavorite {
	...
	my $menu = {
		type        => 'text',
		name        => Slim::Utils::Strings::clientString('FAVORITES_ADDING'),
		showBriefly => 1,
		refresh     => 1,
	};
...
sub trackInfoRemoveFavorite {
	...
	my $menu = [
		{
			type => 'link',
			name => Slim::Utils::Strings::clientString('PLUGIN_FAVORITES_CANCEL'),
			url  => sub {
				my $callback = $_[1];
				
				$callback->( {
					type        => 'text',
					name        => Slim::Utils::Strings::clientString('PLUGIN_FAVORITES_CANCELLING'),
					showBriefly => 1,
					popback     => 2,
				} );
			},
		},
		{
			...
			url  => sub {
				...
				my $menu2 = {
					type        => 'text',
					name        => Slim::Utils::Strings::clientString('FAVORITES_DELETING'),
					showBriefly => 1,
					popback     => 2,
					refresh     => 1,
				};
